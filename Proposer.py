...
class Proposer:
	...
	def __init__ (self, idnum, majority_numb, prev_leader_numb, acceptor, socket_connections_list = None):
		...
		self.acceptor = acceptor # potential bug depending on how python passes parameters
		if socket_connections_list != None:
			self.set_socket_list(socket_connections_list)


	def acceptRequest(self, value, acceptor, seq_number_override=-1):
	...
	def newFollower (self, prev_leaderNum, seq_number, last_value):
		...
		if not self.am_leader: # if not leader
			if (self.numb_followers >= self.majority_numb):
				...
				if (max_prevVal != -1 and max_prevSeqNum != -1 and max_prevLeader != -1):
				...
				while not self.requests_before_leadership.empty():
					self.acceptRequest(self.requests_before_leadership.get(), self.acceptor)
